...
visited1 = [False] * (N + 1)  # dfs의 방문기록 (1)
visited2 = [False] * (N + 1)  # bfs의 방문기록 (2)


def bfs(V):
    q = deque([V])  # pop메서드의 시간복잡도가 낮은 덱 내장 메서드 이용
    visited2[V] = True  # 해당 V 값을 방문처리
    while q:  # q가 빌때까지 돈다.
        V = q.popleft()  # 큐에 있는 첫번째 값 꺼낸다.
        print(V, end=" ")  # 해당 값 출력
        for i in range(1, N + 1):  # 1부터 N까지 돈다
            if not visited2[i] and graph[V][i]:  # 만약 해당 i값을 방문하지 않았고 //V와 연결이 되어 있다면 ########### 이 graph~ 연결부분 재확인###
